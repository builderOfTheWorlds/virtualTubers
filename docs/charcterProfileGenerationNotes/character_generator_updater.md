...
1. **Character generator** — reads source material, interprets it, and
   ...
   UserInput: The knowledge graph will be its own function that generates the actual graph based on the data in the character profile, as a part of the generator we only need to setup the data and the data format, the actual grpah components will be handled by a seperate process. 
