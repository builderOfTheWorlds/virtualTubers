...
| `chadwick` | Half-orc paladin, Oath of Vengeance | `gruff` | That the people irritating him are his siblings |
| `Leena` | Human sorcerer, born **Leena** | `bright` | That her village, her family and her name are all fabricated |
| `Vigil` | Human ranger, 32 | `dry` | That the wood elves who saved him were sent |
| `sodacan_bob` | Dwarf runeseeker of Vabokedos | `deep` | That his apprenticeship was the only placement that looked ordinary on purpose |
...
- `success` → **`magic-retained`**: the seal holds, magic survives thin and
  rationed. Leena is *rare*.
- `failure` → **`magic-lost`**: the well empties, magic dies out of the age.
  Leena is *impossible*, which is a much better question.
...
| `the-age-of-war` | The Event, told straight. Leena's arcana check is the hinge. |
| `magic-retained` / `magic-lost` | Three-to-four beats each. Establish the premise, let Leena react to what she now is. |
| `letos-manor` | The ball. Sodacan Bob reads the joinery, Vigil counts the exits, Chadwick says the quiet part. Grovley appears. |
...
| `burn-it-down` | The cost of the failed fork. Sodacan Bob reads the building's bones; they go down, not out. |
| `grovley-revelation` | The Begene Sisters' program. Vigil's memory of four cribs. *"Leto was father to all of them. To all of you."* |
| `the-vault` | The magic word, the thing holding back the 86th dimension, and the signet ring cut in half. |
| `portal-encounter` | The dying wizard lands his last syllable as Vigil's arrow lands. The tear opens; fingers widen it from inside. |
...
- What the shape in the vault actually is, and why Leena has dreamed it.
