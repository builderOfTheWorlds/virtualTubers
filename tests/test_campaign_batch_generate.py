...
class FakeLLM:
    ...
    def __init__(self, *replies):
        self.replies = list(replies) or ["Leena: A fresh line."]
        self.calls = []
...
def make_pack(scenes=()):
    cast = {
        ...
        "Leena": CastMember(id="Leena", name="Leena", role="player", archetype="wizard"),
    }
    return CampaignPack(
        name="testpack", title="Test", genre="fantasy", start_scene="opening",
        gm_id="gm", player_ids=["Leena"], primitives=[],
...
def test_generate_take_returns_beats_as_dicts():
    ...
    llm = FakeLLM("Leena: We should run.\ngm: The fire crackles.")
...
def test_generate_take_resets_transcript_between_attempts():
    ...
    scene = ambient_scene()
    pack = make_pack([scene])
    llm = FakeLLM("Leena: Fresh every time.")
    improviser = LLMImproviser(pack, llm=llm)
    improviser.observe("Leena", "leftover line from a previous take")
...
def test_write_take_round_trips_through_yaml(tmp_path):
    beats = [{"kind": "dialogue", "speaker": "Leena", "text": "We should run."}]
