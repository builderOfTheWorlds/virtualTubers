...
# tuber_2=Vigil/coder-native, tuber_3=Sodacan Bob/coder-opencode,
# tuber_5=Leena/tester, tuber_6=MAX-1/manager) light up too. manager IS
# mapped here (unlike the request above) so its narrator lines get a tile
# at all instead of falling to the roundtable director's own
# uncast-speaker fallback — mapped to tuber_0 (the "Game Master" tile
# itself, Ashiorid's actual seat), NOT tuber_6/MAX-1: an earlier version of
# this payload mapped manager->tuber_6, which meant tuber_0's own tile
# never owned a single scene and sat silent all show (the GM tile showing
# no dialogue — Gitea-reported bug, fixed here). Fired right after the
# request above so both audiences get the same show - keep this episode
# name in sync with PAYLOAD's above by hand; nothing enforces it
# automatically.
TO2="tuber_0"
TYPE2="replay_request"
